...
https://www.bilibili.com/video/BV1rY4y1r7vA/?spm_id_from=333.337.search-card.all.click&vd_source=11e374f893d48c6126b9e40183afe383
...
pgpool-II保存已经连接到服务器的连接，并在使用相同的参数（如用户名，数据库）连接进来时我不用他们，它减少了开销，并增加了系统的总体吞量。

**2. 负载均衡**

当数据运行在复制模式或主务模式下，pgpool能够将查询语句分散到集群的各个数据库中

**3. 故障切换**

如果其中一台数据服务器出现故障或者无法访问，则pgpool-II会将其分离，并将继续使用其余的数据库服务器进行操作。

**4. 在线还原**

pgpool-II可以通过执行一个命令来执行数据库节点的联机恢复。

**5. 复制**

pgpool-II可以管理多个PostgreSQL服务器。激活复制功能可以在两个或多个PostgreSQL集群上创建实时备份，如果其中一台节点失效，服务可以不被中断继续运行。

**6. 限制超过限度的连接**

PostgreSQL会限制当前的最大连接数，当到达这个数量时，新的连接将会被拒绝。
...
这种模式PgPool不负责后端数据库同步，只负责实现一个故障切换功能。数据库的数所同步由用户负责。这种模式不支持负载均衡。

**2. 主备模式**

使用其他软件完成数据复制，如slony或者流复制，中间件层使用pgpool-II，pgpool-II提供高可用和连接池的功能。

**3. 复制模式**

这种模式下pgpool负责后端数据库数据同步，pgpool节点上的写操作需等待所有后端数据库将数据写入后才向客户端返回成功，是强同步复制模式，这种模式支持负载均衡。
...
![image-20230831125610077](img\image-20230831125610077.png)

  **watchdog的作用** 

1. 和pgpool后端postgreSQL数据节点以及远程pgpool节点进行通信。
2. 对远程pgpool是否存进行检查。
3. 当watchdog子进程启动时。对本地pgpool的配制和远程pgpool的配制参数进行检查，并且输出本地和远程pgpool不一致的参数。
4. 当pgpool主节点宕机时，watchdog集群将选举出新的watchdog主节点。
5. 当pgpool备节点激活成主节点时，watchdog负责将VIP飘移到新的pgpool节点。



**场景1：正常运行时的架构**

![image-20230831195651971](img\image-20230831195651971.png)

**场景2：数据库主节点宕机**



![image-20230831195803334](D:\java\myself\learn\learn-md\pg\img\image-20230831195803334.png)

**场景3：主节点的pgpool和watchdog宕机**

![image-20230831195922716](D:\java\myself\learn\learn-md\pg\img\image-20230831195922716.png)

**场景4：主节点数据库和pgpool都宕机**

![image-20230831200003728](D:\java\myself\learn\learn-md\pg\img\image-20230831200003728.png)
...
#listen_addresses = 'localhost' 改为* 表示对全体的访问进行开放。
listen_addresses = '*'
...
listen_backlog_multiplier = 2
                                   ...
                                   # (change requires restart)
# 是否针对客户端的连接串行化调用accept()的调用过程。默认为false,即不串行化
...
backend_flag0 = 'ALLOW_TO_FAILOVER' # 设置后端数据库节点的行业。当前默认表示允许故障转移。
...
enable_pool_hba = off              # 开启hba的连接认证。
...
log_destination = 'stderr'         # 设置成syslog,在/var/log/message系统日志里会显示pgpool日志。
                                   # Where to log
...
log_line_prefix = '%t: pid %p: '   # 输出到每行日志开头的打印样式字符串。
