    ...
    final class SubscriptionImpl implements Subscription, Runnable {
        ...
        private void doSubscribe() {
            ...
            if (!cancelled) {
                ...
                try {
                    // 此处的this就是Subscription的实现类SubscriptionImpl的对 象。
            ...
            try {
                ...
                do {
                    ...
                    if (!hasNext) {
                        ...
                        // 如果没有取消订阅。This makes  sure that rule 1.8 is upheld,
        ...
        private void terminateDueTo(final Throwable t) {
            ...
            try {
                ...
                (new IllegalStateException(subscriber +
                        " violated the  Reactive Streams rule 2.13 by throwing an exception from onError.", t2))
                        .printStackTrace(System.err);
            }
        }
        // 该方法异步地给订阅票据发送指定信号
         ...
         * 订阅者取消订阅。
         * <p>
         * Subscription.cancel的实现，用于通知Subscription，Subscriber不需要更
         * 多元素了。
         */
