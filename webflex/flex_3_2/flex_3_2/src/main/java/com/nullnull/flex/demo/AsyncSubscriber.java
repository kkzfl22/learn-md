...
public abstract class AsyncSubscriber<T> implements Subscriber<T>, Runnable {
    ...
    private final void handleOnSubscribe(final Subscription s) {
        if (s == null) {
            ...
            try {
                // Cancel the additional subscription to follow rule 2.5
                s.cancel();
            } catch (final Throwable t) {
            ...
            // Because according to rule 3.10, the Subscription is allowed
            // to call `onNext`synchronously from within
            // `request`
            subscription = s;
            try {
                ...
                s.request(1); // Our Subscriber is unbuffered and modest,
    ...
    private final void handleOnNext(final T element) {
        if (!done) { // If we aren't already done
            if (subscription == null) { // Technically this check is not
                ...
                try {
                    if (whenNext(element)) {
                        try {
                            subscription.request(1); // Our Subscriber is
    ...
    // This `ConcurrentLinkedQueue` will track signals that are sent to
    //this `Subscriber`,like `OnComplete`and `OnNext` ,
    // and obeying rule 2.11
    /**
     *
     */
    private final ConcurrentLinkedQueue<Signal> inboundSignals = new
            ...
            try {
                ...
                if (!done) {
                    ...
                    if (s instanceof OnNext<?>)
                        ...
                        // 根据规范2.9，必须处理onComplete信号，不管有没有调用过
                        //Subscription.request( long n)方法
