...
public class RxBookRepository {
  ...
  public Flowable<Book> save(Flowable<Book> books) {
    return books.flatMap(book -> save(book).toFlowable());
  }

  private Single<Book> save(Book book) {
    return database
        .update(SAVE_QUERY)
  ...
  public Flowable<Book> findAll() {
    return database.select(Book.class).get();
  }

  public Maybe<Book> findById(String id) {
    return database.select(SELECT_BY_ID).parameter("id", id).autoMap(Book.class).firstElement();
  }
  ...
  public Flowable<Book> findByYearBetween(Single<Integer> from, Single<Integer> to) {
    return Single.zip(from, to, Tuple2::new)
        .flatMapPublisher(
            tuple ->
                database
