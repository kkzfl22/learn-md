...
```sh
地址：00007ff60694c040-->41
地址：00007ff60694c044-->18467
地址：00007ff60694c048-->6334
地址：00007ff60694c04c-->26500
地址：00007ff60694c050-->19169
地址：00007ff60694c054-->15724
地址：00007ff60694c058-->11478
地址：00007ff60694c05c-->29358
地址：00007ff60694c060-->26962
地址：00007ff60694c064-->24464
```
...
```sh
( 解引用)函数指针：00007ff6c81714a4-->10
(不解引用)函数指针：00007ff6c81714a4-->10
( 解引用)直接调用: 00007ff6c81714a4-->10
(不解引用)直接调用: 00007ff6c81714a4-->10

函数地址：00007ff6c81714a4,解引用后地址:00007ff6c81714a4
函数地址：00007ff6c81714a4,解引用后地址:00007ff6c81714a4
```
...
![image-20251119202315352](.\images\image-20251119202315352.png)
...
```sh
一级指针，地址：000000b8419ffe94,值：150
二级指针，地址：000000b8419ffe88,值:000000b8419ffe94,取一级指针:150
三级指针，地址: 000000b8419ffe80,值:000000b8419ffe88,取二批指针：000000b8419ffe94,取一级指针：150
```
...
（3）带参宏定义的一般形式为#define宏名（形参列表）替换文本，在替换文本中可以含有各个形参。
...
![image-20251207144647668](images\image-20251207144647668.png)

Ø **输入流（Input Stream****）：**数据从数据源（例如文件、键盘等）到程序（内存）的传输路径。

Ø **输出流（Output Stream****）：**数据从程序（内存）到数据源（例如文件、屏幕等）的传输路径。

![image-20251207144735056](images\image-20251207144735056.png)

C 输入 & 输出

（1）当我们提到输入时，这意味着要向程序（内存）中写入一些数据。输入的数据来源可以是从键盘中、从文件中或从网络中。C语言提供了一系列内置的函数来读取给定的输入，并根据需要写入到程序中。

（2）当我们提到输出时，这意味着要在`屏幕`上、`打印机`上或`文件中`显示（或保存）一些数据。同样，C语言提供了一系列`内置的函数`来输出数据到指定的载体上。
...
### 15.3 scanf() 和printf()函数

C语言中的I/O （输入/输出）通常使用`printf()`和`scanf()`两个函数，`scanf()`函数从标准输入流stdin读取输入，`printf()`函数把输出写入到标准输出流stdout。这两个函数我们在前面的案例中已经大量使用。
...
int main(){
    ...
    gets(str); //此处存在一个问题，如果用户输入的字符超过了，str数组的长度，则会出现错误。
...
```c
//正常输入
entrer a line of text :this a t
you entered:this a t
        
```

如果输入的字符超过了长度，则有可能引发错误：

```sh
entrer a line of text :this is a teset 1111111111111222222222222222222222222222222222223456787654321
you entered:this is a teset 1111111111111222222222222222222222222222222222223456787654321
```



![image-20251207225014141](images\image-20251207225014141.png)

如果注意控制台，还有告警输出：

```sh
gets-puts.c:8:5: warning: call to 'gets' declared with attribute warning: Using gets() is always unsafe - use fgets() instead [-Wattribute-warning]
    8 |     gets(str);
      |     ^~~~~~~~~
...
```sh
entrer a line of text :this is a test 1234567890-9543123123123123123123123
